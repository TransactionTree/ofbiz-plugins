...
public class ModelApiReader {
    ...
    public static ModelApi getModelApi(final File apiDef) {
        ...
        for (Element resourceEle : UtilXml.childElementList(docElement, "resource")) {
            Debug.logInfo("resource  " + resourceEle.getNodeName(), MODULE);
    ...
    private static void createOperations(Element resourceEle, ModelResource resource) {
        System.out.println("Operation list "+UtilXml.childElementList(resourceEle, "operation").size());
        for (Element operationEle : UtilXml.childElementList(resourceEle, "operation")) {
            ...
            ModelOperation op = new ModelOperation().path(UtilXml.checkEmpty(operationEle.getAttribute("path")).intern())
            ...
            Debug.logInfo("operation  " + op, MODULE);
        }
    }
